...
- **View** (`OverlayPages` → `OverlayView`): `components/views/<type>.qml`, which receives `screen` and `viewConfig`. `Custom` (`views/Custom.qml`) places its `columns` side by side. `Keybinds`, `BarEditor` and `Settings` are hand-built pages, with their pieces in `views/barEditor/` and `views/settings/`. `ShellManager.showOverlayPage(type)` switches the overlay to a page.
