...
- **Pills** (`BarContainer.pillRects`): one per section, merged when they're at most `pillMerge` apart. Each is an `AttachedSurface` growing out of the bar's outer edge, covering the border stroke (`barConfig.overlap`); widgets sit `overlap + pillPad` in from that edge. A bar popout wider than its pill (`BarPopouts.mergeWithPill`) grows from the outer edge instead, deeper by the pill, with the pill left showing through the surface's notch (`notchStart`/`notchLength`/`notchDepth`, also cut out of the input mask). The pill drops its stroke meanwhile (`mergedPill`).
