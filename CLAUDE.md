...
**Overlay editor:**
- `views/OverlayEditor.qml` with its pieces in `views/overlayEditor/`. It's not in config: `OverlayPages` adds it as a persistent page after the Repeater (and the Themes page), so it's always last, can't be removed, and survives the page rebuild that every config change causes.
